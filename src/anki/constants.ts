...
/**
 * Schema-18 DDL per `docs/formats/anki.md` §Schema-18 DDL, without the
 * `COLLATE unicase` clauses: sql.js cannot register Anki's custom collation,
 * and Anki's importer only SELECTs from package databases, so binary
 * collation is safe for `.apkg` (see the spec's ⚠ note). `notes`, `cards`,
 * and `revlog` keep their schema-11 shape; `col`'s JSON columns are stale
 * empty strings at schema 18.
 */
export const ankiModernDbSchema = `
CREATE TABLE cards (
  id integer PRIMARY KEY,
  nid integer NOT NULL,
  did integer NOT NULL,
