...
- ⚠️ `COLLATE unicase` is a custom collation registered by Anki's Rust code at connection time. Reading rows via sql.js works, but **inserting** rows touches the unicase-collated unique indexes and will fail with "no such collation sequence" unless the writer registers a substitute collation (sql.js `create_collation` support to be verified in the implementation story) or creates the indexes after data insertion via a registered collation. Resolve before writing the schema-18 writer.
