...
- Reading via sql.js needs the schema-text patch from Story 1.3.6 (strip `COLLATE unicase` via `PRAGMA writable_schema` before querying — the `tags` table is unreadable otherwise)
...
- [x] ~~Resolve the `unicase` collation question first (spike)~~ — resolved in Story 1.3.6: sql.js has no collation support; the writer creates the DDL without `COLLATE unicase` (binary collation), which Anki's importer provably accepts for `.apkg` (see `docs/formats/anki.md` §Schema-18 DDL ⚠️)
